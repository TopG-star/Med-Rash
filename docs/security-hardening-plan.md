...
_Phase 3c — strip the gate key entirely (follow-up commit after Phase 3b ships clean for one pilot session):_

- [ ] Remove the gate-key fallback branch from `device-token.ts`. Make `turnstileToken` a required body field.
- [ ] Delete `admin/netlify/functions/_shared/gate.ts`.
- [ ] Remove `MEDRASH_GATE_API_KEY` from Netlify env.
- [ ] Drop `_gateApiKey` field from Flutter `DeviceTokenStore`, `MedRashHttpClient`, `AppConfig` constructors. `init_core.dart` rewired to omit the param.
- [ ] Drop the gate-key `x-medrash-gate-key` header attachment in `MedRashHttpClient._buildHeaders`.
...
- Phase 3b hosted smoke (Turnstile challenge succeeds, `/device-token` rejects without Turnstile, rate limit kicks in after burst) — **PENDING user redeploy with `MEDRASH_TURNSTILE_SECRET` + `MEDRASH_TURNSTILE_SITE_KEY` (--dart-define) set.**
- Phase 3c hosted smoke (gate key fully removed, only Turnstile accepted) — **PENDING Phase 3c implementation.**
