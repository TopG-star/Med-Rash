...
**Status:** Phase 1 ✅ shipped 2026-05-29 (foundation: dep + 6 schemas + helper + 43 tests, zero wire-ins). Phase 2 ✅ shipped 2026-05-29 (9/10 Netlify functions gated by zod front door + structured `INVALID_INPUT` issues[]; `quiz-list` has no body — N/A). Phase 3 pending.
...
- [ ] **Phase 3:** Wire schemas into Next.js server actions (`admin/src/app/login/actions.ts`, `admin/src/app/admin-users/actions.ts`, `admin/src/app/onboarding/actions.ts`, `admin/src/app/sessions/actions.ts`, `admin/src/app/quiz-bank/actions.ts`). Retire shared parsers (`parseIdentityInput`, `parseCreateSessionInput`, `parseCreate*Input`) now that both call sites are zod-gated.
...
**Verification phase 1+2:** typecheck PASS · vitest 117/117 PASS (74 prior + 43 new schema tests; existing function suites unaffected) · lint 0 errors / 14 pre-existing warnings (1 introduced by this slice was cleaned up before commit). Phase 3 will add: integration test that a malformed POST to `/attempt-submit` returns structured 400 with field-level `issues[]`.
