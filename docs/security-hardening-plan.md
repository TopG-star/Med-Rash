...
- [ ] Phase 2 (after ~24h hosted observation with zero blocking CSP-Report-Only violations in console): rename both header keys from `Content-Security-Policy-Report-Only` → `Content-Security-Policy` in `netlify.toml`, `app/netlify.toml`, and `admin/next.config.ts` (3 edits, one commit). Redeploy.
...
**Verification:** Phase 1 (this commit) — admin `npm run typecheck` PASS · Netlify deploy preview surfaces all 6 headers (HSTS + XFO + nosniff + Referrer-Policy + Permissions-Policy + CSP-Report-Only) on every response · admin browse (login → dashboard → quiz-bank → sessions → reports → intelligence → admin-users) shows zero blocking violations in DevTools console (only Report-Only warnings if any) · participant browse (QR scan or join code → quiz → leaderboard) shows zero blocking violations · Cloudflare Turnstile widget still mints tokens · Supabase Realtime still connects. Phase 2 — same browse paths after CSP flip to enforcing PASS · securityheaders.com grade A on both origins.
