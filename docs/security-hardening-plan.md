...
**Status:** Phase 1 ✅ shipped 2026-05-29 (foundation: dep + 6 schemas + helper + 43 tests, zero wire-ins). Phases 2–3 pending.
...
- [ ] **Phase 2:** Wire schemas into 10 Netlify functions (`attempt-submit`, `session-create`, `session-resolve`, `quiz-bank-write`, `quiz-list`, `leaderboard`, `profile-sync`, `ranked-eligibility`, `recover-request`, `recover-verify`); delete handwritten validators (`parseIdentityInput`, `parseCreateSessionInput`, `parseAnswers`, etc.). Standard 400 response with field-level issues.
- [ ] **Phase 3:** Wire schemas into Next.js server actions (`admin/src/app/login/actions.ts`, `admin/src/app/admin-users/actions.ts`, `admin/src/app/onboarding/actions.ts`, `admin/src/app/sessions/actions.ts`, `admin/src/app/quiz-bank/actions.ts`).
...
**Verification phase 1:** typecheck PASS · vitest 117/117 PASS (74 prior + 43 new schema tests). Phase 2/3 will add: manual POST malformed JSON to `/attempt-submit` returns structured 400 + existing endpoint integration tests still PASS.
