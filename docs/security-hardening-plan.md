...
- [ ] **Phase 2:** Wire `logAuthEvent` into remaining auth surfaces: `requestOtpAction`, `signOutAndRedirectAction`, `getAdminSession` (allowlist deny path), `recover-request`, `recover-verify`.
- [ ] **Phase 2:** Wire `logAdminAction` into remaining admin-write surfaces: `quiz-bank-write` (7 ops — `create_quiz`, `update_quiz`, `deactivate_quiz`, `create_question`, `update_question`, `deactivate_question`, `bulk_create_questions`), `admin-users/actions.ts` (5 ops — `invite_admin`, `reinvite_admin`, `deactivate_admin`, `reactivate_admin`, `set_admin_role`), `onboarding/actions.ts` (1 op — `complete_onboarding`).
...
**Verification phase 1:** typecheck PASS · vitest PASS (73/73, +7 new) · `supabase db push` PENDING (user-driven) · hosted smoke PENDING — trigger one OTP verify success + one session create, then `select event_type, count(*) from app.auth_events group by 1` and `select action, count(*) from app.admin_audit group by 1` should each return ≥1 row.
...
- **2026-05 — Slice A4 hotfix: Flutter CanvasKit + Roboto CSP allowances.** Hosted browser hit a hard CSP block on first paint: Flutter's default CanvasKit loader fetches `canvaskit.js` + `canvaskit.wasm` from `https://www.gstatic.com/flutter-canvaskit/<hash>/`, and the Roboto fallback hits `https://fonts.gstatic.com/s/roboto/.../woff2`. Both were absent from the participant `app/netlify.toml` CSP. Patch added `https://www.gstatic.com` to `script-src` + `connect-src`, and `https://fonts.gstatic.com` to `connect-src` + `font-src` (both directives needed: Fetch API call + browser font-face binding). Alternative we did NOT take: `flutter build web --no-web-resources-cdn` to vendor CanvasKit into `build/web` — would let us drop the gstatic allowance but adds ~2MB to the deploy and complicates the build script. Deferred until we decide to vendor; meanwhile gstatic.com is well-scoped (Google-owned, HTTPS-only, no user-controlled subdomain wildcard). Tighten later if compliance requires Google-free egress.
