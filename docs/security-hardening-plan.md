...
- [x] Add Sentry (or equivalent) to admin Next.js and Flutter participant. PII scrubber on by default; bind release SHA.
- [x] Define alerting thresholds (error-rate spike, auth-fail spike).
- **Files touched (B7):** `admin/instrumentation.ts`, `admin/instrumentation-client.ts`, `admin/sentry.server.config.ts`, `admin/sentry.edge.config.ts`, `admin/src/lib/observability/sentry-scrubber.ts` (+ test), `admin/next.config.ts` (CSP `connect-src` + `withSentryConfig` wrap), `admin/package.json` / `package-lock.json`, `netlify.toml` (CSP), `app/netlify.toml` (CSP), `app/pubspec.yaml` / `pubspec.lock`, `app/lib/main.dart`, `app/lib/core/observability/sentry_scrubber.dart` (+ test), `app/scripts/build-web.sh`, `docs/security/alerting.md`, `docs/dev-environment.md`, `docs/security-hardening-plan.md`.
...
- **2026-05 — Slice B7 (client-side telemetry: Sentry admin + Flutter, shared PII scrubber, alerting doc).** Adds `@sentry/nextjs@^10` (8.x rejected on Next 16 peer-dep; 9 also unverified — latest stable 10.55.0 declares `next: ^16.0.0-0` directly) and `sentry_flutter@^9.1.0` to participant. **Empty-DSN posture** mirrors B1 P1's `MEDRASH_ADMIN_SESSION_SECRET`: every runtime checks `process.env.SENTRY_DSN` / `String.fromEnvironment('SENTRY_DSN')` and silently skips `init()` when blank, so the slice merges before the Sentry org exists. **Three Next.js runtimes wired separately** — `instrumentation-client.ts` (browser, new Sentry v9+ convention), `instrumentation.ts` → `sentry.server.config.ts` (nodejs runtime) and `→ sentry.edge.config.ts` (edge runtime; covers the B1 middleware), gated by `NEXT_RUNTIME` so an Edge bundle does not pull in node-only Sentry modules. Re-exports `captureRequestError as onRequestError` from `instrumentation.ts` (Next 15+ contract; Sentry v10 renamed the export). `withSentryConfig` wraps the existing `nextConfig` with `tunnelRoute: "/monitoring"` (first-party proxy to Sentry ingest so adblockers don't drop events) and `sourcemaps.disable: !SENTRY_AUTH_TOKEN` so local + PR-preview builds without Sentry creds still succeed. **Shared PII scrubber** in `admin/src/lib/observability/sentry-scrubber.ts` + `app/lib/core/observability/sentry_scrubber.dart` enforces identical rules on every runtime: strips `user.email` / `user.username` / `user.ip_address`; drops cookies + Authorization + Cookie headers; truncates strings > 2048 chars; redacts `[email-redacted]` substrings inside exception values + breadcrumb messages; strips query string + fragment from request URLs and from `fetch`/`xhr`/`navigation` breadcrumb URLs (`url`/`from`/`to` fields). Layered on top of `sendDefaultPii: false` — the scrubber is defence-in-depth, not the primary control. Both scrubbers covered by unit tests (admin: 7/7 vitest; Flutter: 6/6 flutter_test). **TS types sourced from `@sentry/core`** (`@sentry/nextjs` does not re-export `TransactionEvent`/`Breadcrumb` in v10) — noted only so the next consumer doesn't waste time hunting the same export. **Flutter scrubber rewrite**: sentry_flutter 9.x deprecated `copyWith` in favour of direct field assignment, so the scrubber mutates fields in place; `SentryRequest.headers` getter returns `Map.unmodifiable(...)` so the scrubber rebuilds the headers map then assigns via the `set headers(...)` setter (cannot call `.updateAll` directly on the unmodifiable view); `SentryRequest` has no `data` setter, so request bodies are not nulled here — the SDK does not populate them under `sendDefaultPii=false` anyway. **Sampling**: tracesSampleRate `0.1` everywhere (start conservative, tune in Sentry org); `replaysSessionSampleRate` + `replaysOnErrorSampleRate` both `0` on the admin browser because Session Replay captures full DOM including any PII rendered on screen (email, attempt scores, participant names) — explicit re-enable gated on a privacy review with selector-based masking. **CSP dual-layer update** per A4 discipline: `connect-src` extended with `https://*.ingest.sentry.io https://*.ingest.us.sentry.io` in `admin/next.config.ts`, `netlify.toml` (admin) AND `app/netlify.toml` (participant) in the same commit so the framework + CDN layers stay in lockstep. The `tunnelRoute: /monitoring` is the primary path; the ingest origin allowance is a fallback for when the tunnel is disabled by env override or fails to deploy. **Flutter env wiring**: `app/scripts/build-web.sh` forwards `--dart-define=SENTRY_DSN=...`, `SENTRY_RELEASE` (defaults to Netlify `COMMIT_REF`), `SENTRY_ENVIRONMENT` (defaults to Netlify `CONTEXT`), all optional. **Alerting doc** `docs/security/alerting.md` enumerates page-worthy thresholds per source (Sentry: new issue / regressed / error-rate spike `>50 events / 5min` / crash-free sessions `<99%`; Supabase: `app.auth_events` failure burst `>20/min/IP` or `>200/hr` org-wide — routed through Supabase Database Webhook because Sentry never sees these; CI: dep-audit + gitleaks block merges) with channel routing pinned to the single canonical env var `INCIDENT_PRIMARY_EMAIL` from IR doc — Slack/PagerDuty deferred until the team grows past one on-caller, with the future P0/P1/P2 routing already documented so the upgrade is a config change not a doc rewrite. **No Replay, no Profiling, no Performance Insights** enabled — they each add data-flow risk that needs its own privacy review; the slice ships the minimum that satisfies the plan stub. **Verification:** admin typecheck PASS (exit 0), vitest 152/152 PASS (+7 new for sentry-scrubber), lint 0 errors / 14 pre-existing warnings (unchanged baseline); `flutter analyze` PASS (no issues), `flutter test` 186/186 PASS (+6 new for sentry_scrubber).
