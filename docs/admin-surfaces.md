...
| `MEDRASH_ADMIN_SESSION_SECRET` | admin Next.js (middleware + signout route) | **Required (Slice B1 phase 1).** ≥32-char random string. HMAC-SHA256 key (Web Crypto API, isomorphic Edge + Node) for the `medrash-admin-session` cookie that enforces the agreed session timeout policy on every authenticated admin request: idle 30 min + absolute 8 h. On timeout, middleware clears both this cookie and the Supabase `sb-*-auth-token` cookies, then redirects to `/login?reason=session_idle` or `?reason=session_absolute`. If unset, the middleware fails closed and redirects authenticated requests to `/denied?reason=config` rather than letting them through without timeout enforcement. Rotation has the same posture as `MEDRASH_DEVICE_TOKEN_SECRET`: all in-flight admin cookies fail signature verification on the next request and the middleware reinitialises them (the admin user stays signed in via Supabase but the session-timeout clock restarts). |
