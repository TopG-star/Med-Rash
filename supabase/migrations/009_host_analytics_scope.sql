-- 009_host_analytics_scope.sql
-- Phase C: expand Host access. Add `created_by_filter uuid` to the three
-- analytics RPCs so Reports + Intelligence can scope to sessions a given
-- user (Host) created. Owners pass NULL and see workspace-wide aggregates.
--
-- Backward compatibility: callers that did not pass the new parameter
-- continue to work because it defaults to NULL. We drop the old function
-- signatures first to avoid ambiguous-function errors when PostgREST
-- resolves overloads.

drop function if exists app.knowledge_gaps(integer, text, text, uuid);
drop function if exists app.facility_performance(integer);
drop function if exists app.treatment_perception_trends(integer);
